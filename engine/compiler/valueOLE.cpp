...
static std::map<IDispatch *, CValueOLE *> aOLEValues;
...
{
#ifdef __WXMSW__
	for (auto dispOle : aOLEValues)
	{
		CValueOLE *m_oleValue = dispOle.second;
		if (!m_oleValue->m_streamThreading) continue;
		HRESULT hr = ::CoGetInterfaceAndReleaseStream(m_oleValue->m_streamThreading, IID_IDispatch, (void **)&m_oleValue->m_dispatchThreading);
		if (SUCCEEDED(hr)) m_oleValue->m_streamThreading = NULL;
	}
#endif
}
...
void CValueOLE::CreateStreamForDispatch()
{
	...
	for (auto dispOle : aOLEValues)
	{
		CValueOLE *m_oleValue = dispOle.second;
		if (m_oleValue->m_streamThreading) continue;
		HRESULT hr = ::CoMarshalInterThreadInterfaceInStream(IID_IDispatch, m_oleValue->m_dispatch, &m_oleValue->m_streamThreading);
		if (FAILED(hr)) m_oleValue->m_streamThreading = NULL;
	}
#endif
}
...
void CValueOLE::ReleaseCoObjects()
{
	...
	{
		CValueOLE *m_oleValue = dispOle.second;
		if (!m_oleValue->m_dispatch) continue;
		unsigned int countRef = m_oleValue->m_dispatch->Release();
		for (unsigned int i = 0; i < countRef; i++) { m_oleValue->m_dispatch->Release(); }
		m_oleValue->m_dispatch = NULL;
	}
#endif
}
...
m_clsId({ 0 }),
m_dispatch(NULL), m_dispatchThreading(NULL), m_streamThreading(NULL),
...
m_clsId(clsId),
m_dispatch(dispatch), m_dispatchThreading(NULL), m_streamThreading(NULL),
m_methods(new CMethods()),
m_sObjectName(objectName)
{
	if (m_dispatch) m_dispatch->AddRef();
	PrepareNames();
}
...
{
	if (appData->IsDesignerMode()) return true;
	...
	if (wxThread::IsMain()) aOLEValues.insert_or_assign(m_dispatch, this);
...
{
#ifdef __WXMSW__
	if (!m_dispatch) return; unsigned int count = 0;
	if (m_dispatch->GetTypeInfoCount(&count) != S_OK) return;
			...
			typeAttr->typekind == TKIND_DISPATCH)
		{
			for (unsigned int m = 0; m < (UINT)typeAttr->cFuncs; m++)
			{
				...
				for (unsigned int k = 0; k < (UINT)funcInfo->cParams; k++)
				{
					...
					if (k < (UINT)funcInfo->cParams - 1) sMethodDescription += wxT(", ");
				}

				sMethodDescription += wxT(")");

				if (funcInfo->invkind == INVOKE_FUNC) aMethods.push_back({ sMethod , sMethodDescription });
				else if (funcInfo->invkind == INVOKE_PROPERTYGET) aAttributes.push_back({ sMethod });
			}
		}
...
int CValueOLE::FindAttribute(const wxString &sName) const 
{
	...
	if (wxThread::IsMain() || !m_dispatchThreading)
	{
		if (!m_dispatch) return wxNOT_FOUND;
		...
		if (hr != S_OK || dispid == 1000) return wxNOT_FOUND;
		return dispid;
	}
	else
	{
		if (!m_dispatchThreading) return wxNOT_FOUND;

		LPOLESTR cbstrName = (WCHAR *)sName.wc_str();
		DISPID dispid = -1;
		OLECHAR FAR* szMember = cbstrName;

		HRESULT hr = m_dispatchThreading->GetIDsOfNames(
			IID_NULL,
			&szMember,
			1, LOCALE_SYSTEM_DEFAULT,
			&dispid);
...
CValue CValueOLE::GetAttribute(CAttributeParameters &aParams)
{
#ifdef __WXMSW__
	...
	if (wxThread::IsMain()
		|| !m_dispatchThreading)
	{
		if (!m_dispatch) return CValue();

		m_dispatch->Invoke(
			aParams.GetAttributeIndex(),
			IID_NULL,
	...
	{
		if (!m_dispatchThreading) return CValue();

		m_dispatchThreading->Invoke(
			aParams.GetAttributeIndex(),
			IID_NULL,
			LOCALE_SYSTEM_DEFAULT,
			DISPATCH_PROPERTYGET,
...
void CValueOLE::SetAttribute(CAttributeParameters &aParams, CValue &cVal)
{
#ifdef __WXMSW__
	if (wxThread::IsMain()
		|| !m_dispatchThreading)
	{
		...
		PutProperty(m_dispatch, aParams.GetAttributeIndex(), &var);
		FreeValue(var);
	}
	else
	{
		if (!m_dispatchThreading) return;
		VARIANT var = FromValue(cVal);
		PutProperty(m_dispatchThreading, aParams.GetAttributeIndex(), &var);
		FreeValue(var);
	}
#endif 
}
...
CValue CValueOLE::Method(CMethodParameters &aParams)
{
#ifdef __WXMSW__
	if (!m_dispatch) return CValue();
	...
	for (unsigned int i = 0; i < nParamCount; i++) { pvarArgs[nParamCount - i - 1] = FromValue(aParams[i]); }
	...
	if (wxThread::IsMain()
		|| !m_dispatchThreading)
	{
		hr = m_dispatch->Invoke(aParams.GetMethodIndex(),
			IID_NULL,
			LOCALE_SYSTEM_DEFAULT,
	...
	{
		hr = m_dispatchThreading->Invoke(aParams.GetMethodIndex(),
			IID_NULL,
			LOCALE_SYSTEM_DEFAULT,
			DISPATCH_METHOD | DISPATCH_PROPERTYGET,
			&dispparams, &varRet, &ExcepInfo, &uiArgErr);
	...
	for (unsigned int i = 0; i < nParamCount; i++) FreeValue(pvarArgs[i]);
