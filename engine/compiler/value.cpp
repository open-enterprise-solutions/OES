	...
	: ITypeValue(type), m_refCount(0), m_pRef(NULL), m_bReadOnly(readOnly)
{
	...
	case TYPE_DATE: m_dData = 0; break;
...
void CValue::operator = (eValueTypes type)
{
	...
	if (m_typeClassOld != eValueTypes::TYPE_EMPTY) SetData(objValue);
}
...
void CValue::SetDate(const wxString &sDate)
{
	wxDateTime strTime; CValue cRes = eValueTypes::TYPE_DATE;

	if (strTime.ParseDateTime(sDate))
	{
	...
	{
		wxString sYear = sDate.Left(4);
		wxString sMonth = sDate.Mid(4, 2);
		wxString sDay = sDate.Mid(6, 2);

		if (sDate.Length() > 8)
		{
			int year = wxAtoi(sYear);
			wxDateTime::Month month = (wxDateTime::Month)(wxAtoi(sMonth) - 1);
			unsigned short day = wxAtoi(sDay);

			wxString sHour = sDate.Mid(8, 2);
			wxString sMinute = sDate.Mid(10, 2);
			wxString sSeconds = sDate.Mid(12, 2);

			unsigned short hour = wxAtoi(sHour);
			unsigned short minute = wxAtoi(sMinute);
			unsigned short seconds = wxAtoi(sSeconds);

			if ((month < wxDateTime::Month::Dec && day > 0 && day <= wxDateTime::GetNumberOfDays(month, year))
				&& (hour < 24 && minute < 60 && seconds < 60))
			{	
				strTime.Set(day, month, year, hour, minute, seconds);
		...
		else
		{
			...
			if (month < wxDateTime::Month::Dec
				&& day <= wxDateTime::GetNumberOfDays(month, year))
			{
				strTime.Set(day, month, year);
...
{
	wxDateTime m_dateTime(00, 00, 00);

	m_dateTime.SetDay(01);
	m_dateTime.SetMonth(wxDateTime::Month::Jan);
	m_dateTime.SetYear(0001);

	switch (m_typeClass)
	{
	case eValueTypes::TYPE_BOOLEAN:
		return m_dateTime.GetValue().GetValue();
	case eValueTypes::TYPE_NUMBER:
	{
	...
	case eValueTypes::TYPE_STRING:
	{
		wxDateTime dateTime(00, 00, 00);

		dateTime.SetDay(01);
		dateTime.SetMonth(wxDateTime::Month::Jan);
		dateTime.SetYear(0001);

		if (dateTime.ParseDateTime(m_sData))
			return dateTime.GetValue().GetValue();
		break;
	}
	case eValueTypes::TYPE_DATE:
		return m_dData;
...
CValue CValue::Method(const wxString &sName, CValue **aParams)
{
	...
	if (iName >= 0)
	{
		CMethodParameters aMethParams(aParams, sizeof(aParams) / sizeof(aParams[0]), iName, sName);
...
CValue CValue::Method(CMethodParameters &aParams)
{
	GET_THIS
		if (aParams.GetMethodIndex() >= 0)
		{
...
void CValue::SetAttribute(CAttributeParameters &aParams, CValue &cVal)
{
	GET_THIS0
		if (aParams.GetAttributeIndex() < 0 || pThis == 0)
			CTranslateError::Error(_("Value does not represent an aggregate object"));
	pThis->SetAttribute(aParams, cVal);
}
...
CValue CValue::GetAttribute(CAttributeParameters &aParams)
{
	GET_THIS0
		if (aParams.GetAttributeIndex() >= 0 && pThis)
			return pThis->GetAttribute(aParams);
	CTranslateError::Error(_("Value does not represent an aggregate object"));
	return *this;
}
...
CValue CValue::GetValue(bool bThis)
{
	...
	if (m_typeClass == eValueTypes::TYPE_REFFER) return m_pRef->GetValue(true);// true - признак создания новой переменной - ссылки на агрегатный объект
