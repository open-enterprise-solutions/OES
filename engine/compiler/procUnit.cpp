...
static CErrorPlace m_errorPlace;

void CProcUnit::Raise()
{
	m_errorPlace.Reset(); //инициализация места ошибки
	m_errorPlace.pSkipByteCode = CProcUnit::GetCurrentByteCode(); //возвращаемся назад в вызываемый модуль (если он есть)
}
	...
	CStackGuard(CRunContext *pContext)
	{
		if (s_nRecCount > MAX_REC_COUNT)//критическая ошибка
		{
			...
			for (unsigned int i = 0; i < CProcUnit::GetCountRunContext(); i++)
			{
				...
				sError += wxString::Format("\n%s (#line %d)", m_pByteCode->m_sModuleName, m_pByteCode->m_aCodeList[pLastContext->m_nCurLine].m_nNumberLine + 1);
			}
...
inline void ModValue(CValue &cValue1, const CValue &cValue2, const CValue &cValue3)
{
	...
	if (cValue1.m_typeClass == eValueTypes::TYPE_NUMBER)
	{
		...
		if (val128_3.IsZero()) CTranslateError::Error(_("Divide by zero"));
...
{
	//проверка доступности переменной и контроль ссылок
	if (cValue1.m_bReadOnly) { cValue1.SetValue(bValue); return; }
...
{
	//проверка доступности переменной и контроль ссылок
	if (cValue1.m_bReadOnly) { cValue1.SetValue(fValue); return; }
...
{
	m_aParent.clear();
	if (!pSetParent) return;
	unsigned int nCount = pSetParent->m_aParent.size();
	m_aParent.push_back(pSetParent);
	for (unsigned int i = 1; i <= nCount; i++) { m_aParent.push_back(pSetParent->m_aParent[i - 1]); }
}
...
#ifdef _DEBUG
	if (!pContext) {
		CTranslateError::Error(_("No execution context defined!")); if (!m_pByteCode) CTranslateError::Error(_("No execution code set!"));
	}
#endif
...
start_label:
	...
	try { //медленнее на 2-3% на каждый вложенный модуль
		while (nCodeLine < nFinish) {
			...
			//enter in debugger
			if (!CTranslateError::IsSimpleMode()) {
				debugClient->EnterDebugger(pContext, CurCode, nPrevLine);
			}
			...
			case OPER_AND: if (IsHasValue(Variable2) && IsHasValue(Variable3)) SetTypeBoolean(Variable1, true); else SetTypeBoolean(Variable1, false); break;
			case OPER_OR: if (IsHasValue(Variable2) || IsHasValue(Variable3)) SetTypeBoolean(Variable1, true); else SetTypeBoolean(Variable1, false); break;
			...
			case OPER_IF: if (IsEmptyValue(Variable1)) nCodeLine = Index2 - 1; break;
			case OPER_FOR:
			{
				if (Variable1.m_typeClass != eValueTypes::TYPE_NUMBER) CTranslateError::Error(_("Only variables with type can be used to organize the loop \"number\""));
				if (Variable1.m_fData == Variable2.m_fData) nCodeLine = Index3 - 1; break;
			}
			case OPER_FOREACH:
			{
				if (!Variable2.HasIterator()) CTranslateError::Error(_("Undefined value iterator"));
				if (Variable3.m_typeClass != eValueTypes::TYPE_NUMBER) { CopyValue(Variable3, CValue(0)); }
				unsigned int m_nIndex = Variable3.ToUInt();
				if (m_nIndex < Variable2.GetItSize()) CopyValue(Variable1, Variable2.GetItAt(m_nIndex));
				else { Variable3 = 0; nCodeLine = Index4 - 1; } break;
			}
			case OPER_NEXT: if (Variable1.m_typeClass == eValueTypes::TYPE_NUMBER) { Variable1.m_fData++; } nCodeLine = Index2 - 1; break;
			case OPER_NEXT_ITER: Variable1.m_fData++; nCodeLine = Index2 - 1; break;
			case OPER_ITER: if (IsHasValue(Variable2)) CopyValue(Variable1, Variable3); else CopyValue(Variable1, Variable4); break;
			case OPER_NEW:
			{
			...
			case OPER_SET_A://установка атрибута
			{
				...
				CAttributeParameters aParams(nAttr, sAttributeName);
				Variable1.SetAttribute(aParams, GetValue(Variable3)); break;
			}
			...
			case OPER_CALL_M://вызов метода
			{
				...
				if (pStorageValue && pStorageValue == pVariable2->GetRef())//ранее были вызовы
				{
					...
					if (nMethod != Index4) CTranslateError::Error(_("Error value %d must %d (It is recommended to turn off method optimization)"), Index4, nMethod);
#endif
				}
				...
				if (nMethod < 0) CheckAndError(Variable2, sFuncName);
				...
				CMethodParameters methodParams(cRunContext.m_pRefLocVars, cRunContext.m_nParamCount, nMethod, sFuncName);
				CopyValue(*pRetValue, pVariable2->Method(methodParams)); pRetValue->CopyValue(); methodParams.CheckParams(); break;
			}
	...
	{
		CSystemObjects::Message(err->what(), eStatusMessage::eStatusMessage_Error);
	...
	catch (const CTranslateError *err)
	{
		...
		if (n >= 0)
		{
			m_errorPlace.Reset();//Ошибка обрабатывается в этом модуле - стираем место ошибки
		...
		if (!m_errorPlace.m_pByteCode)
		{
			if (m_pByteCode != m_errorPlace.pSkipByteCode)//системная функция Ошибка выдает исключение только для дочерних модулей
			{
				//ранее сохранили оргинально место ошибки (т.е. ошибка произошла не в этом модуле)
				m_errorPlace.m_pByteCode = m_pByteCode;
				m_errorPlace.nLine = nCodeLine;
			}
		}

		if (!CTranslateError::IsSimpleMode())
			CTranslateError::ProcessError(m_pByteCode->m_sFileName, m_pByteCode->m_sModuleName, m_pByteCode->m_sModule, m_pByteCode->m_aCodeList[nCodeLine].m_nNumberLine, m_pByteCode->m_aCodeList[nCodeLine].m_nNumberString);
		else throw(err);
	}
...
CValue CProcUnit::Execute(CByteCode &cByteCode, bool bRunModule)
{
	if (!cByteCode.m_bCompile) {
		CTranslateError::Error(_("Module: %s not compiled!"), cByteCode.m_sModuleName);
	}
	...
	if (GetParent())
	{
		...
		{
			m_pByteCode = NULL;
			CTranslateError::Error(_("System error - compilation failed (#1)\nModule:%s\nParent1:%s\nParent2:%s"), cByteCode.m_sModuleName.wc_str(), cByteCode.m_pParent->m_sModuleName.wc_str(), GetParent()->m_pByteCode->m_sModuleName.wc_str());
		}
	}
	else if (m_pByteCode->m_pParent)
	{
		m_pByteCode = NULL;
		CTranslateError::Error(_("System error - compilation failed (#2)\nModule1:%s\nParent1:%s"), cByteCode.m_sModuleName.wc_str(), cByteCode.m_pParent->m_sModuleName.wc_str());
	}
...
{
	int nCodeLine = 0;
	if (!m_pByteCode || !m_pByteCode->m_bCompile) CTranslateError::Error(_("Module not compiled!"));
	if (bExportOnly) nCodeLine = m_pByteCode->m_aExportFuncList[StringUtils::MakeUpper(sName)] - 1;
	else nCodeLine = m_pByteCode->m_aFuncList[StringUtils::MakeUpper(sName)] - 1;
	if (bError && nCodeLine < 0) CTranslateError::Error(_("Procedure or function \"%s\" not found!"), sName.wc_str());
	if (nCodeLine >= 0) return nCodeLine;
	if (GetParent() && bExportOnly <= 1)
	{
		...
		if (nCodeLine >= 0) return nCodeSize + nCodeLine;
	}
	return wxNOT_FOUND;
}
...
CValue CProcUnit::CallFunction(unsigned int nCodeLine, CValue **ppParams, unsigned int nReceiveParamCount)
{
	...
	{
		if (GetParent()) return GetParent()->CallFunction(nCodeLine - nCodeSize, ppParams, nReceiveParamCount);
...
CValue CProcUnit::CallFunction(CMethodParameters &aParams)
{
	if (!m_pByteCode || !m_pByteCode->m_bCompile) {
		CTranslateError::Error(_("Module not compiled!"));
	}

	int nCodeLine = m_pByteCode->m_aFuncList.find(aParams.GetMethodName(true)) != m_pByteCode->m_aFuncList.end() ? m_pByteCode->m_aFuncList[aParams.GetMethodName(true)] - 1 : wxNOT_FOUND;
...
CValue CProcUnit::CallFunction(const wxString &sName, CValue **ppParams, unsigned int m_nParamCount)
{
	if (m_pByteCode)
	{
		int nCodeLine = m_pByteCode->m_aFuncList.find(StringUtils::MakeUpper(sName)) != m_pByteCode->m_aFuncList.end() ? m_pByteCode->m_aFuncList[StringUtils::MakeUpper(sName)] - 1 : wxNOT_FOUND;
		if (nCodeLine >= 0) return CallFunction(nCodeLine, ppParams, m_nParamCount);
	}
...
void CProcUnit::SetAttribute(CAttributeParameters &aParams, CValue &cVal)//установка атрибута
{
	*m_cCurContext.m_pRefLocVars[aParams.GetAttributeIndex()] = cVal;
}
...
CValue CProcUnit::GetAttribute(CAttributeParameters &aParams)//значение атрибута
{
	return *m_cCurContext.m_pRefLocVars[aParams.GetAttributeIndex()];
}
...
CValue CProcUnit::Evaluate(const wxString &sExpression, CRunContext *pRunContext, bool bCompileBlock, bool *bError)
{
	CValue cRetValue;

	if (!pRunContext) pRunContext = CProcUnit::GetCurrentRunContext();
	if (sExpression.IsEmpty() || !pRunContext) return CValue();

	bool m_isSimpleMode = CTranslateError::IsSimpleMode();
	if (!m_isSimpleMode) CTranslateError::ActivateSimpleMode();
	...
	if (pRunContext->m_aEvalString.find(StringUtils::MakeUpper(sExpression)) == pRunContext->m_aEvalString.end()) //еще не было компиляции такого текста
	{
		...
		if (!m_pSimpleRun->CompileExpression(sExpression, pRunContext, *m_compileModule, bCompileBlock))
		{
			...
			if (!m_isSimpleMode) CTranslateError::DeativateSimpleMode();
	...
	if (m_module->m_bExpressionOnly)
	{
		...
		while (m_curContext)
		{
			if (!m_curModule->m_bExpressionOnly) break;
			m_curContext = m_curContext->m_parentContext; m_curModule = m_module->GetParent();
		}

		if (m_curContext && m_curContext->m_nReturn == RETURN_NONE) nDelta = 1;
	}
	else
	{
		if (m_compileContext->m_nReturn == RETURN_NONE) nDelta = 1;
	}
...
bool CProcUnit::CompileExpression(const wxString &sExpression, CRunContext *pRunContext, CCompileModule &cModule, bool bCompileBlock)
{
	...
	if (!bCompileBlock) cModule.m_cByteCode.m_aCodeList.push_back(code);
...
CRunContext::~CRunContext()
{
	...
	{
		CProcUnit *m_pRun = static_cast<CProcUnit*>(it->second);
		if (m_pRun)
		{
			CCompileModule *m_pModule = m_pRun->m_pByteCode->m_pModule;
			delete m_pRun; delete m_pModule;
		}
	}
}