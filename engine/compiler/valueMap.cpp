...
void CValueContainer::CValueReturnContainer::SetAttribute(CAttributeParameters &aParams, CValue & cValue)
{
}

CValue CValueContainer::CValueReturnContainer::GetAttribute(CAttributeParameters &aParams)
{
...
void CValueContainer::SetAttribute(CAttributeParameters &aParams, CValue &cVal)
{
	SetAt(aParams.GetAttributeName(), cVal);
}

CValue CValueContainer::GetAttribute(CAttributeParameters &aParams)
{
	return GetAt(aParams.GetAttributeName());
}

CValue CValueContainer::Method(CMethodParameters &aParams)
{
...
void CValueContainer::Insert(const CValue &cKey, CValue &cValue)
{
	...
	if (itFound != m_containerValues.end()) { if (!appData->IsDesignerMode()) { CTranslateError::Error(_("Key '" + cKey.GetString() + "' is already using!")); } return; }
	SetAt(cKey, cValue);
}
...
CValue CValueContainer::GetAt(const CValue &cKey)
{
	...
	if (!appData->IsDesignerMode()) { CTranslateError::Error(_("Key '" + cKey.GetString() + "' not found!")); }
	return CValue();
}
...
CValue CValueStructure::GetAt(const CValue &cKey)
{
	...
	{
		if (!appData->IsDesignerMode()) { CTranslateError::Error(st_error_conversion); } return CValue();
	}
...
void CValueStructure::SetAt(const CValue &cKey, CValue &cValue)
{
	if (cKey.GetType() != eValueTypes::TYPE_STRING)
	{
		if (!appData->IsDesignerMode()) { CTranslateError::Error(st_error_conversion); } return;
	}
...
bool CValueStructure::Property(const CValue &cKey, CValue &cValueFound)
{
	if (cKey.GetType() != eValueTypes::TYPE_STRING)
	{
		if (!appData->IsDesignerMode()) { CTranslateError::Error(st_error_conversion); } return false;
	}
