...
#include "elements/baseControl.h"
...
IControlElement *IVisualHost::GetObjectBase(wxObject* wxobject)
{
	if (NULL == wxobject)
	{
	...
	{
		wxLogError(_("No corresponding IControlElement for wxObject. Name: %s"), wxobject->GetClassInfo()->GetClassName());
		return NULL;
	}
}
...
wxObject* IVisualHost::GetWxObject(IControlElement* baseobject)
{
	if (!baseobject)
	{
	...
	{
		wxLogError(_("No corresponding wxObject for IControlElement. Name: %s"), baseobject->GetClassName().c_str());
		return NULL;
	}
}

void IVisualHost::DeleteRecursive(IControlElement *control, bool force)
{
	for (unsigned int i = 0; i < control->GetChildCount(); i++) { DeleteRecursive(control->GetChild(i), force); }
	...
	if (control->GetComponentType() == COMPONENT_TYPE_WINDOW)
	{
		...
		if (controlWnd) {
			...
			m_baseObjects.erase(control); m_wxObjects.erase(controlWnd);

			wxWindow *controlParent = controlWnd->GetParent();
			controlWnd->DestroyChildren(); controlWnd->Destroy();
			if (!force && controlParent) controlParent->Layout();
		}
	}
	else if (control->GetComponentType() == COMPONENT_TYPE_SIZER)
	{
		...
		if (controlSizer) {
			...
			IControlElement *controlParent = control->GetParent();
			...
			if (controlParentSizer) controlParentSizer->Detach(controlSizer);
			wxDELETE(controlSizer);
			if (!force && controlParentSizer) controlParentSizer->Layout();
		}
	}
	else
	{
...
void IVisualHost::CreateControl(IControlElement *obj, IControlElement *parent)
{
	IControlElement *objControl = obj; IControlElement *objParent = parent ? parent : obj->GetParent();
	...
	IControlElement* nextParent = objParent;
	...
	GenerateControl(objControl, windowObj, parentObj);
	//and update it 
...
void IVisualHost::UpdateControl(IControlElement *obj, IControlElement *parent)
{
	IControlElement *objControl = obj; IControlElement *objParent = parent ? parent : obj->GetParent();
	wxWindow *windowObj = NULL; wxObject *parentObj = NULL; wxWindow *hostWnd = GetBackgroundWindow();
...
void IVisualHost::RemoveControl(IControlElement *obj, IControlElement *parent)
{
	IControlElement *objControl = obj; IControlElement *objParent = parent ? parent : obj->GetParent();
	wxWindow *windowObj = NULL; wxObject *parentObj = NULL; wxWindow *hostWnd = GetBackgroundWindow();
...
void IVisualHost::GenerateControl(IControlElement *obj, wxWindow* wxparent, wxObject* parentObject)
{
	// Create Object
	...
	// Associate the wxObject* with the IControlElement*
	...
	{
		GenerateControl(obj->GetChild(i), new_wxparent, createdObject);
	}

	//if new obj then call on created 
	OnCreated(obj, createdObject, wxparent);
...
void IVisualHost::RefreshControl(IControlElement *obj, wxWindow* wxparent, wxObject* parentObject)
{
	// Create Object
