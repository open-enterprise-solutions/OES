...
{
	if (aParams[0]->GetType() == eValueTypes::TYPE_STRING) { m_colour = TypeConv::StringToColour(aParams[0]->ToString()); return true; }
	else { m_colour = wxColour(aParams[0]->ToInt(), aParams[1]->ToInt(), aParams[2]->ToInt()); return true; }
	return false;
}
...
void CValueColour::SetAttribute(CAttributeParameters &aParams, CValue &cVal)
{
...
CValue CValueColour::GetAttribute(CAttributeParameters &aParams)
{
	switch (aParams.GetAttributeIndex())
	{
