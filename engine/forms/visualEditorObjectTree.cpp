...
IControlElement *CVisualEditorContextForm::CVisualEditorObjectTree::GetObjectFromTreeItem(wxTreeItemId item)
{
	if (item.IsOk())
	{
		...
		if (item_data)
		{
			IControlElement * obj(((CVisualEditorObjectTreeItemData*)item_data)->GetObject());
			return obj;
		}
	}
...
void CVisualEditorContextForm::CVisualEditorObjectTree::RebuildTree()
{
	...
	IControlElement *valueFrame = m_formHandler->GetSelectedForm();
	...
	{
		wxTreeItemId dummy;
		AddChildren(valueFrame, dummy, true);
		// Expand items that were previously expanded
		RestoreItemStatus(valueFrame);
	}
...
void CVisualEditorContextForm::CVisualEditorObjectTree::OnBeginDrag(wxTreeEvent &event)
{
	...
	if (event.GetItem() == m_tcObjects->GetRootItem()) return;
...
void CVisualEditorContextForm::CVisualEditorObjectTree::OnEndDrag(wxTreeEvent& event)
{
	...
	while (item.IsOk())
	{
		if (item == itemSrc) return;
		item = m_tcObjects->GetItemParent(item);
	}

	IControlElement *objSrc = GetObjectFromTreeItem(itemSrc);
	if (!objSrc) return;

	IControlElement *objDst = GetObjectFromTreeItem(itemDst);
	if (!objDst) return;

	// backup clipboard
	IControlElement *clipboard = m_formHandler->GetClipboardObject();

	// set object to clipboard
	if (copy) m_formHandler->CopyObject(objSrc);
	else m_formHandler->CutObject(objSrc, true);

	if (!copy && !m_formHandler->PasteObject(objDst)) { m_formHandler->Undo(); }
...
void CVisualEditorContextForm::CVisualEditorObjectTree::AddChildren(IControlElement *obj, wxTreeItemId &parent, bool is_root)
{
	if (obj->IsItem())
	{
		...
		else
		{
			...
			wxString msg;
			IControlElement *itemParent = obj->GetParent();
			assert(parent);
	...
	else
	{
		...
		else
		{
			...
			IControlElement *parent_obj = obj->GetParent();
			...
			if (pos > 0) new_parent = m_tcObjects->InsertItem(parent, pos, wxT(""), wxNOT_FOUND, wxNOT_FOUND, item_data);
			else new_parent = m_tcObjects->AppendItem(parent, wxT(""), wxNOT_FOUND, wxNOT_FOUND, item_data);
		}
		...
		if (image_idx != wxNOT_FOUND) m_tcObjects->SetItemImage(new_parent, image_idx);
		...
		{
			IControlElement *child = obj->GetChild(i);
...
int CVisualEditorContextForm::CVisualEditorObjectTree::GetImageIndex(wxString name)
{
	int index = 0; //default icon

	IconIndexMap::iterator it = m_iconIdx.find(StringUtils::MakeUpper(name));
	if (it != m_iconIdx.end())
		index = it->second;
...
void CVisualEditorContextForm::CVisualEditorObjectTree::UpdateItem(wxTreeItemId id, IControlElement *obj)
{
	// mostramos el nombre
...
void CVisualEditorContextForm::CVisualEditorObjectTree::Create()
{
	...
	//for (auto obj_info : visualEditorDatabase->GetAvailableClasses())
	//{
	//	if (obj_info.m_bitmap.IsOk())
	//	{
	//		m_iconList->Add(obj_info.m_bitmap);
	//		m_iconIdx.insert(IconIndexMap::value_type(obj_info.m_classname, index++));
	//	}
	//}
...
void CVisualEditorContextForm::CVisualEditorObjectTree::RestoreItemStatus(IControlElement * obj)
{
...
void CVisualEditorContextForm::CVisualEditorObjectTree::AddItem(IControlElement *item, IControlElement *parent)
{
	if (item && parent)
	{
		// find parent item displayed in the object tree
		while (parent && parent->IsItem()) parent = parent->GetParent();
...
void CVisualEditorContextForm::CVisualEditorObjectTree::RemoveItem(IControlElement *item)
{
...
void CVisualEditorContextForm::CVisualEditorObjectTree::ClearMap(IControlElement * obj)
{
	m_map.erase(obj);
...
void CVisualEditorContextForm::CVisualEditorObjectTree::OnPropertyModified(wxFramePropertyEvent &event)
{
	...
	{
		ObjectItemMap::iterator it = m_map.find((IControlElement *)prop->GetObject());
...
CVisualEditorContextForm::CVisualEditorObjectTreeItemData::CVisualEditorObjectTreeItemData(IControlElement * obj) : m_object(obj) {}
...
CVisualEditorContextForm::CVisualEditorItemPopupMenu::CVisualEditorItemPopupMenu(CVisualEditorContextForm *handler, wxWindow *parent, IControlElement *obj)
