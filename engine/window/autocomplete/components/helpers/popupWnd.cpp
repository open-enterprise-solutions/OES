...
{
	// Although we're a valueFrame, we always want the parent to be active,
	// so raise it whenever we get shown, focused, etc.
	wxTopLevelWindow *valueFrame = wxDynamicCast(
...
wxSTCListBoxWin::wxSTCListBoxWin(wxWindow* parent, wxSTCListBoxVisualData *visualData, int h)
	: wxSTCPopupWindow(parent), m_visualData(visualData)
{
	m_listBox = new wxSTCListBox(this, m_visualData, h);

	// Use the background of this window to form a valueFrame around the listbox
	// except on macos where the native Scintilla popup has no valueFrame.
#ifdef __WXOSX_COCOA__
