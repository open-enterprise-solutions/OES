		...
		if (endSeg > startSeg) {
			...
			else
			{
				...
				if (draw)
				{
					...
					dc.DrawText(wxString::FromUTF8(s, len), wxRound(rcClient.GetLeft()), wxRound(static_cast<float>(ytext) - font.GetAscent()));
...
void CCallTip::Show(int currentPos, const wxString &sTitleText)
{
	...
	position.x = wxRound(position.x + rc.GetLeft());
	position.y = wxRound(position.y + rc.GetTop());
...
bool CCallTip::ProcessEvent(wxEvent &event)
{
