...
void CAutocomplectionCtrl::AddKeywordFromObject(const CValue &vObject)
{
	...
	else if (debugServer->IsEnterLoop())
	{
		CPrecompileContext *m_pContext = m_precompileModule->GetCurrentContext();

		if (m_pContext && m_pContext->FindVariable(m_precompileModule->sLastParentKeyword)) {
			ac.Cancel();
			debugServer->EvaluateAutocomplete(this, m_precompileModule->sLastExpression, m_precompileModule->sLastKeyword, GetCurrentPos());
		}
	}
}
...
bool CAutocomplectionCtrl::PrepareExpression(unsigned int currPos, wxString &sExpression, wxString &sKeyWord, wxString &sCurrWord, bool &hasPoint)
{
	...
	for (unsigned int i = 0; i < m_precompileModule->m_aLexemList.size(); i++)
	{
		...
		{
			sCurrWord = m_precompileModule->m_aLexemList[i].m_vData.GetString();
			m_hasKeyword = CompareString(sKeyWord, wxT("type")) || CompareString(sKeyWord, wxT("showCommonForm")) || CompareString(sKeyWord, wxT("getCommonForm")) && !m_hasPoint;
		}
...
void CAutocomplectionCtrl::CalculateFoldLevels()
{
	...
	for (unsigned int line = 0; line < (unsigned int)GetLineCount(); line++)
	{
		...
		eFoldStates foldState = eFoldStates::eNormalFold;
		...
		if (foldState == eFoldStates::eNormalFold) { SetFoldLevel(line, wxSTC_FOLDLEVELBASE + nFoldLevels); }
	}
}
...
void CAutocomplectionCtrl::LoadToolTip(wxPoint pos)
{
	if (debugServer->IsEnterLoop())
	{
		...
		else if (!sExpression.IsEmpty()) debugServer->EvaluateToolTip(sModuleName, sExpression);
	}
}
		...
		if (m_hasPoint)
		{
			...
			if (m_precompileModule->Compile())
			{
				...
				for (unsigned int i = 0; i < vObject.GetNMethods(); i++)
				{
					...
					if (CompareString(sMethod, sCurWord)) {
					...
					if (computeModuleObject)
					{
						...
						if (cParser.ParseModule(computeModuleObject->GetModuleText()))
						{
							for (auto code : cParser.GetAllContent())
							{
								...
								{
									if (CompareString(code.sName, sCurWord)) {
		...
		else
		{
			...
			if (m_precompileModule->Compile())
			{
				...
				for (auto function : m_pContext->cFunctions)
				{
					...
					if (CompareString(function.first, sCurWord)) {
...
{
	if (CompareString(keyWord, wxT("new")))
	{
	...
	else if (CompareString(keyWord, wxT("type")))
	{
	...
	else if (CompareString(keyWord, wxT("showCommonForm"))
		|| CompareString(keyWord, wxT("getCommonForm")))
	{
