...
void CMainFrame::OnActivateView(bool activate, wxView *activeView, wxView *deactiveView)
{
	if (m_toolbarAdditional)
	{
		...
		if (m_deactiveView) { m_deactiveView->OnRemoveToolbar(m_toolbarAdditional); m_toolbarAdditional->ClearTools(); }

		if (m_deactiveDocTemplate != m_activeDoc->GetDocumentTemplate())
		{
			if (activate) m_activeView->OnCreateToolbar(m_toolbarAdditional);

			if (m_toolbarAdditional->GetToolCount() > 0) m_infoToolBar.Show();
