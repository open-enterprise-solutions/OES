...
class wxWatchWindowDataEvent : public wxEvent
{
	...
	struct itemData_t
	{
		...
		itemData_t(wxString n, wxString v, wxString t, bool a, wxTreeItemId i) : name(n), value(v), type(t), hasAttributes(a), itemID(i) {}
	};
...
public:
	...
	void AddWatch(wxString name, wxString value, wxString type, bool hasAttributes);
	void AddWatch(wxString name, wxString value, wxString type, bool hasAttributes, wxTreeItemId item);
