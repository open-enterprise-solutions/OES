...
#if defined(_USE_64_BIT_POINT_IN_DEBUG)
	if (!m_expression.IsEmpty()) debugServer->AddExpression(m_expression, reinterpret_cast<u64>(item.GetID()));
#else 
	if (!m_expression.IsEmpty()) debugServer->AddExpression(m_expression, reinterpret_cast<u32>(item.GetID()));
#endif 
...
void CWatchWindow::OnItemDeleted(wxTreeEvent& event)
{
	wxTreeItemId item = event.GetItem();
#if defined(_USE_64_BIT_POINT_IN_DEBUG)
	if (!m_updating) debugServer->RemoveExpression(reinterpret_cast<u64>(item.GetID()));
#else 
	if (!m_updating) debugServer->RemoveExpression(reinterpret_cast<u32>(item.GetID()));
#endif 
	event.Skip();
}
...
#if defined(_USE_64_BIT_POINT_IN_DEBUG)
	if (!m_updating) debugServer->ExpandExpression(sExpression, reinterpret_cast<u64>(item.GetID()));
#else 
	if (!m_updating) debugServer->ExpandExpression(sExpression, reinterpret_cast<u32>(item.GetID()));
#endif 
...
void CWatchWindow::OnDataExpanding(wxWatchWindowDataEvent& event)
{
	...
	for (unsigned int i = 0; i < event.GetWatchCount(); i++)
	{
		...
		if (event.HasAttributes(i)) AppendItem(child, wxT("..."));
	}
...
void CWatchWindow::OnDataSetVariable(wxWatchWindowDataEvent& event)
{
	...
	for (unsigned int i = 0; i < event.GetWatchCount(); i++)
	{
		...
		if (event.HasAttributes(i)) AppendItem(item, wxT("..."));
	}
